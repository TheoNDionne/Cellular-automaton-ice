...
class CoordsFromIceMap:
    ...
    def coords_from_array_position(self, line, col):
    ...
    def construct_coords_from_ice_map(self, ice_map):
        ...
        for line in range(self.L):
            if ice_map[line, 0] == True: # checks if ice cell
                x, y = self.coords_from_array_position(line, 0) 
        ...
        for col in range(1, self.W):
            for line in range(self.L-2*col):
                if ice_map[line, col] == True: # checks if ice cell
                    x, y = self.coords_from_array_position(line, col) 
    ...
    def rotate_coordinate_array(self, vectors, angle):
    ...
    def reconstruct_all_coordinates(self, vectors):
        ...
        for i in range(5):
            all_coordinates[(i+1)*number_of_vectors:(i+2)*number_of_vectors, :] = self.rotate_coordinate_array(vectors, angles[i])
    ...
    def convert_ice_map_to_all_coords(self, ice_map):
        ...
        in the shape produced by construct_coords_from_ice_map().   
        """
        upper_sixth_positions = self.construct_coords_from_ice_map(ice_map) # generate positions for the upper sixth
        all_positions = self.reconstruct_all_coordinates(upper_sixth_positions) # turn sixth into entire coordinates for all cells

        return all_positions

if __name__ == "__main__":
    gamig = CoordsFromIceMap(101)
    print(gamig.coords_from_array_position(0,0))
